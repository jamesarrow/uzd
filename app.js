/* UZI Compare — light compact static MVP (no species/weight)
...
function renderMeta(){
  ...
  els.noteRight.textContent = "Ссылка учитывает фильтры и выбранные клиники.";
}
...
async function init(){
  ...
  const existing = new Set(state.data.clinics.map(c=>c.id));
  state.selectedClinicIds = (state.selectedClinicIds || []).filter(x=>existing.has(x));
  if (!state.selectedClinicIds.length) state.selectedClinicIds = state.data.clinics.map(c=>c.id);
